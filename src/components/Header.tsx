            ...
            return (
              <a
                ...
                className={`relative text-sm font-semibold transition-all duration-300 flex items-center gap-1.5 ${
                  item.isHighlighted
                    ? 'px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-cyan-500/20 via-indigo-500/25 to-cyan-400/20 text-cyan-300 border border-cyan-400/50 hover:border-cyan-300 hover:scale-105 shadow-[0_0_15px_rgba(34,211,238,0.35)]'
                    : isActive
              ...
              >
                ...
                {item.badgeText && (
                  <span className="px-1.5 py-0.5 rounded-md bg-gradient-to-r from-cyan-400 to-indigo-500 text-slate-950 text-[9px] font-mono font-black tracking-wide shadow-sm flex items-center gap-0.5 animate-pulse">
                    <Sparkles className="w-2.5 h-2.5 inline text-slate-950" /> {item.badgeText}
                  </span>
                )}
                {item.isNew && (
                ...
                {item.isExternal && <ArrowUpRight className="w-3.5 h-3.5 text-cyan-300" />}
                {isActive && (
                  <motion.div
            ...
            <div className="flex flex-col items-center gap-6 text-center">
              {menuItems.map((item, index) => (
                <motion.a
                  ...
                  rel={item.isExternal ? 'noreferrer' : undefined}
                  onClick={() => setIsOpen(false)}
                  className={`text-xl sm:text-2xl font-semibold transition-all flex items-center gap-2 ${
                    item.isHighlighted
                      ? 'px-5 py-2.5 rounded-2xl bg-gradient-to-r from-cyan-500/20 via-indigo-500/20 to-cyan-400/20 text-cyan-300 border border-cyan-400/50 shadow-[0_0_20px_rgba(34,211,238,0.4)]'
                      : 'text-slate-200 hover:text-cyan-400'
                  }`}
                >
                  {item.label}
                  {item.badgeText && (
                    <span className="px-2 py-0.5 rounded-md bg-gradient-to-r from-cyan-400 to-indigo-500 text-slate-950 text-xs font-mono font-black animate-pulse tracking-wide shadow-sm flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-slate-950" /> {item.badgeText}
                    </span>
                  )}
                  {item.isNew && (
                  ...
                  {item.isExternal && <ArrowUpRight className="w-5 h-5 text-cyan-300" />}
                </motion.a>
              ))}
